...
const TargetAudienceSection = () => {
  const { open } = usePopup();
  return (
    <section className="section-plain py-24 md:py-32 relative">
      {/* Watermark */}
      <div className="nsm-watermark right-8 top-1/2 -translate-y-1/2">nsm</div>

      <div className="container mx-auto px-6 relative">
        <div className="max-w-3xl mx-auto">
          <p className="section-label">(05) Para quem é</p>

          <h2 className="font-display text-2xl md:text-3xl font-normal mb-12">
            Esse plano é para <span className="text-nlm-blue">você que:</span>
          </h2>
          ...
          <div className="space-y-4 mb-14">
            {criterios.map((criterio, index) => (
              <div
                key={index}
                className="card-outline flex items-start gap-4 p-5"
              >
                <div className="w-7 h-7 rounded-full bg-nlm-positive-bg flex items-center justify-center shrink-0 mt-0.5">
                  <Check className="w-4 h-4 text-nlm-positive" />
                </div>
                <div>
                  <span className="font-medium text-nlm-title">{criterio.text}</span>
                  <span className="text-nlm-secondary ml-1">{criterio.detail}</span>
                </div>
              </div>
            ))}
          </div>
